...
N_interview and applied uniformly to all N_survey households at simulation time. There is no survey-only shortcut for any of
...
- **R-E — Intermittent occupancy (real mobility).** Unlike sedentary Profile 2, these households leave —
  ...
  which is quantified as `prob_home` — [OCC] and simulated by the occupancy mask (§6; Protocol §10) — not a
  per-appliance parameter.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| VA1 Morning indoor light | LED_1 | [240, 420] | Flexible (engine-constrained) | 3 W | 126 | 63 | 0.20 | 0.30 | 0.35 |
| VA2 Daytime placeholder | LED_1 | [420, 1080] | — (engine artifact) | — | — | — | — | — | — (no YAML block; not passed to RAMP) |
| VA3 Evening indoor light | LED_1 | [1080, 1320] | Flexible (engine-constrained) | 3 W | 168 | 84 | 0.20 | 0.30 | 0.90 |
| VA4 Overnight "hacer amanecer" | LED_1 | [1320, 1440] ∪ [0, 240] | Strict (engine-constrained) | 3 W | 288 | 240 | 0.10 | 0.15 | 0.5 |
| VA5 Second-room morning light | LED_2 | [240, 420] | Flexible (engine-constrained) | 2 W | 126 | 63 | 0.10 | 0.20 | 0.35 |
| VA6 Second-room daytime placeholder | LED_2 | [420, 1080] | — (engine artifact) | 2 W | 30 | 15 | 0.30 | 0.35 | 0 |
| VA7 Second-room evening light | LED_2 | [1080, 1320] | Flexible (engine-constrained) | 2 W | 168 | 84 | 0.20 | 0.30 | 1 |
| VA8 Second-room overnight | LED_2 | [1320, 1440] ∪ [0, 240] | Strict (engine-constrained) | 2 W | 288 | 200 | 0.10 | 0.15 | 0.35 |
| VA9 Portable-device charging | USB | [0, 1440] | Chaos | 2.5 W | 480 | 240 | 0.30 | 0 | 1.00 |
...
- **occasional_use:** **0.35** — **[FREQ-XW]** *Occasional/moderate* bin. Morning code present in 13/14 (window is common), so the practice stays in the moderate bin —
...
### VA2 — Daytime indoor light (LED_1) 
...
- **random_var_w:** **0** — **[RIG-XW]** Strict; **lowered to 0 to avoid midnight jump.
- **occasional_use:** **0.33** — **[FREQ-XW]**, *household-level heterogeneity, no stated cause*,
  **moved to the interview-only reading, 2026-07-28** (from 0.25). **Underlying evidence:** survey code 13
  ...
  1)    reports an overnight code. Any-source prevalence = **3/14 ≈ 0.21**; interview-only = **3/9 ≈ 0.33**.
...
One rigidity bin less than the other LED.
...
**Narrative.** Shadows VA1: the same pre-dawn/early-morning rise, in the second room or kitchen, switched on less often because not every household lights both bulbs. Also because we assume one light is used for cooking and the other is not.
...
- **occasional_use:** **0.15** — **[FREQ-XW]** = one less frequent bin that the other morning light
...
⚠ **VA8 must carry this same override**, for the reason given under VA3's: group members share
`occasional_use`, so an override on one has to be mirrored on the other or the seasonal split inverts.
...
1. **The dominant seasonal mechanism is *structural absence*, not behavioural tuning.** Profile 3 is
   ...
   **household-level structural-absence mechanism** (suppress *all* VAs for the absent period), **not** a
   dozen edited `occasional_use` values. This is what the occupancy block below now does, at
   `prob_home = 0.78` for Jul–Sep.
...
**Occupancy — [OCC]: the household-level mask IS applied, and is this profile's primary seasonal mechanism.**
...
This is the parameter point 1 above has been deferring. `prob_home` is the probability that a household of
this category is present on a given day; one draw is made per household per day and it suppresses **every**
VA of that household simultaneously, which is precisely the structural-absence semantics R-E asks for and
which no combination of per-VA `occasional_use` edits can reproduce (independent per-appliance draws produce
incoherent days — some VAs of one routine running while others don't).

**Derivation.** Only the two households meeting the Protocol's diagnostic bar — an explicit multi-week/month
departure, not an inference from `migration_label` — enter the arithmetic:

| household | stated absence | source |
|---|---|---|
| Martín Salazar (78) | *"a veces seis meses"* → 6 months | R-E; the Protocol's named Profile-3 structural-absence anchor |
| Apolina Vallejos (7) | 3 months in Chapare, SHS left completely unused (*"Estuve durante tres meses y no hice alumbrar"*) | R-E; corroborated by **[FIELD MEMO, caseid 53]** |

Total anchored absence is 6 + 3 = **9 household-months** against an interviewed base of 9 × 12 = **108
household-months**, i.e. 8.3% of household-days absent year-round. §6.1 places migration in the Jul–Sep
grazing window, so that absence is concentrated rather than uniform: Apolina's 3 months fit the window
entirely, and at most 3 of Martín's 6 can (the window is only 3 months long), giving **6 of the 9
household-months inside Jul–Sep and 3 outside** — a two-thirds concentration that falls out of the anchors
themselves rather than being asserted.

- **Free grazing (Jul–Sep):** 6 absent household-months ÷ (9 households × 3 months = 27) = 22% absent →
  `prob_home = 0.78`.
- **All other seasons:** 3 absent household-months ÷ (9 households × 9 months = 81) = 3.7% absent →
  `prob_home = 0.96` (the block's baseline).

**Read as an upper bound on presence.** Four further households are named in R-E as genuinely mobile —
Marcelino (*monte*), Primitivo's family (the mine), Basilio (Saipina), Felipe Rivera's kin (Chapare) — with
no stated duration, so their absence is real but uncounted here. The true `prob_home` is therefore at or
below these values, never above them, consistent with the Protocol's directional finding that self-reported
presence overstates actual presence.

**`occasional_use_basis: present_conditional`** — the §5 `occasional_use` values are **not** rescaled when
the mask is switched on. They are read as *fraction of days the household is home*, which is what they
already measure: R-A and R-B describe the occupied-household rhythm, and a respondent describing their
evening routine is describing the evenings they are there. Combined effect on any given day is therefore
`P(VA runs) = prob_home × occasional_use`, and mean annual energy falls by roughly the absence fraction —
which is the intended result, not an artifact. (The alternative, `all_days`, would divide each
`occasional_use` by `prob_home` to hold the mean fixed and change only the variance; it is not used here.)

**⚠ Overlap with §5's provisional override set, in Free Grazing — unresolved, flagged rather than fixed.**
Point 2 above describes that set as covering "Feb–Jun," but the YAML fences actually declare
`seasons: [growing, free_grazing]` for VA1, VA3, VA5, VA7 and VA9 — so the set covers **Feb–Apr and
Jul–Sep**, not Feb–Jun. That matters here, because Jul–Sep is precisely where the mask bites:

- **Growing (Feb–Apr):** no meaningful interaction. `prob_home = 0.96` trims 4%; the per-VA overrides carry
  the seasonal effect, as intended.
- **Free grazing (Jul–Sep):** the two mechanisms act on the same season. VA3, for example, resolves to
  `occasional_use = 0.28` *and* `prob_home = 0.78`, giving `P(runs) = 0.22` — a reduction applied twice for
  one underlying cause, since point 1 above states plainly that Jul–Sep absence should be represented by the
  mask and "**not** a dozen edited `occasional_use` values."

The clean resolution is to drop the `free_grazing` half of those five override fences and let `prob_home`
carry that season alone, keeping the `growing` half untouched. It is not applied here because it changes
five VAs on a judgement about how much of each 0.28 was absence and how much was reduced in-home activity
during the growing season — and because §5 already flags the whole set as a provisional visualization
variant rather than a re-derived finding, so the honest first step is to decide the set's status, not to
edit its numbers. Until then, read Profile 3's Jul–Sep load as **under**-stated. Carried in §8.
...
- [x] **2026-08-18 — the structural-absence occupancy feature landed and is wired here, per §6.** `prob_home`
      is now a parsed, simulated parameter (`User.prob_home` in RAMP; extracted from §6's `yaml occupancy`
      block). This profile carries `0.96` baseline / `0.78` in free grazing, derived from the ids 7 and 78
      anchors — the "wire it here first rather than editing behavioural parameters" instruction below is
      satisfied, and no §5 `occasional_use` value was touched to represent absence.
...
- [ ] **The Free-Grazing half of §5's provisional override set now double-counts absence against `prob_home`, and §6 point 2 misstates which seasons the set covers.** The fences declare `[growing, free_grazing]`, not the "Feb–Jun" the prose claims — so VA1/3/5/7/9's overrides land on Jul–Sep, the same season the mask suppresses. VA3 resolves to `0.28 × 0.78 = 0.22`, one cause priced twice, against §6 point 1's explicit instruction that Jul–Sep absence belongs to the mask and not to per-VA edits. Resolution: decide the override set's status first (provisional visualization variant, or a claim the file stands behind), then either drop the `free_grazing` half of the five fences — leaving `growing` untouched, since `prob_home = 0.96` there — or re-derive each 0.28 as an explicitly present-conditional value. Profile 3's Jul–Sep load is under-stated until this is settled. Note the same defect was found and fixed in Profile 4's VA3 on 2026-08-18; this is the remaining instance.
- [ ] **§6's `prob_home` counts only 2 of the 6 mobile households, so it is an upper bound on presence.** The arithmetic uses ids 78 (six months) and 7 (three months) — the only two absences with a stated duration. R-E names four more genuinely mobile households (Marcelino/*monte*, Primitivo's family/mine, Basilio/Saipina, Rivera's kin/Chapare) whose absence is real but unquantified and therefore contributes nothing to the 0.78/0.96 figures. Getting a duration for any of the four would lower both values; none can raise them. Worth one targeted follow-up question ("how many weeks/months last year?") rather than a re-derivation.
