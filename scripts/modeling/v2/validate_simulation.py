...
Metrics
-------
Tier 1 (Error):        RMSE, nRMSE (RMSE / real mean power), CV of measured
                       (natural-variability reference for nRMSE), LCSS score,
                       MPDADA ratio
LDC (Duration-sorted):  LDC-RMSE (Tier 1's RMSE, computed on the duration-sorted
                       curve instead of the chronological one), nLDC-RMSE,
                       P95 power (W)
Tier 2 (Structure):    Modal peak hour, average-curve peak, absolute max power,
                       MRSD chaos index
Tier 2 (Peak):         Peak time of mean curve (continuous) + signed offset,
                       daily peak-time spread (rigidity signal), load factor
                       (mean/peak), peak width at >=80% of own peak (shape) and
                       at >=80% of the real peak (time-above-threshold)
...
def calculate_structural_metrics(df: pd.DataFrame, label: str = "series") -> dict:
    ...
    peak_ts = df.loc[daily_peak_idx.dropna(), "timestamp"]
    peak_hours = peak_ts.dt.hour
    modal_peak_hour = int(peak_hours.mode()[0]) if not peak_hours.empty else 0

    # Continuous per-day peak instant (decimal hours). Captures the timing
    # detail that the integer modal-hour discards: the mean tells you where
    # the peak lands on average, the std how much it scatters day to day
    # (a rigidity signal -- a schedule-bound profile has a small std, a
    # chaotic one a large one). Assumes peaks do not straddle midnight,
    # which holds here (all profiles peak in the evening window).
    peak_time_decimal = peak_ts.dt.hour + peak_ts.dt.minute / 60.0
    peak_time_mean = float(peak_time_decimal.mean()) if not peak_time_decimal.empty else 0.0
    peak_time_std  = float(peak_time_decimal.std())  if not peak_time_decimal.empty else 0.0
    ...
    return {
        ...
        "peak_time_mean": peak_time_mean, "peak_time_std": peak_time_std,
...
def calculate_peak_metrics(
    ...
    r: dict,
    s: dict,
    peak_frac: float = 0.80,
) -> dict:
    """
    Peak-focused metrics computed on the representative (time-binned) daily
    curve, complementing the single-number peak height already reported
    (Average Curve Peak / P95 / Absolute Max). These characterize the peak
    beyond its height -- its timing, its sharpness/width, and the profile's
    overall peakiness -- which are the quantities that actually drive off-grid
    sizing (inverter rating, storage) but that a peak-magnitude error alone
    does not capture.

    Returned quantities
    -------------------
    peak_time_real/sim (h)   : clock position of the peak of the *mean* curve
                               (continuous, from the curve argmax).
    peak_time_offset_min     : signed (real - sim) offset of that peak, in
                               minutes, wrapped to [-720, 720]. A finer-grained
                               alternative to the integer modal-hour difference.
    load_factor_real/sim     : mean load / peak load on the representative
                               curve (dimensionless, 0-1). Low = spiky (stresses
                               storage), high = flat. A compact "peakiness"
                               signature that also cross-checks MRSD.
    peak_width_own_real/sim  : hours the curve sits at or above `peak_frac` of
                               its *own* average-curve peak -- i.e. how broad the
                               peak is, independent of its height (shape only).
    peak_width_ref_real/sim  : hours each curve sits at or above `peak_frac` of
                               the *real* peak -- i.e. how much time each spends
                               near the real peak *level* (absolute; the
                               sizing-relevant "time-above-threshold"). For the
                               real series this equals peak_width_own_real by
                               construction; the sim value is the one to compare.
    """
    n_real = len(real_profile)
    n_sim  = len(sim_profile)
    bin_h_real = 24.0 / n_real if n_real else 0.25
    bin_h_sim  = 24.0 / n_sim  if n_sim  else 0.25

    # Peak position of the mean curve (continuous hours)
    peak_time_real = float(np.argmax(real_profile) * bin_h_real) if n_real else 0.0
    peak_time_sim  = float(np.argmax(sim_profile)  * bin_h_sim)  if n_sim  else 0.0
    offset_h = peak_time_real - peak_time_sim
    offset_h = (offset_h + 12.0) % 24.0 - 12.0  # wrap to [-12 h, +12 h]
    peak_time_offset_min = offset_h * 60.0

    # Load factor = mean / peak on the representative curve
    lf_real = (r["overall_p_mean"] / real_avg_peak) if real_avg_peak > 0 else 0.0
    lf_sim  = (s["overall_p_mean"] / sim_avg_peak)  if sim_avg_peak  > 0 else 0.0

    # Peak width, self-referenced (>= peak_frac of each curve's own peak)
    width_own_real = float(np.sum(real_profile >= peak_frac * real_avg_peak) * bin_h_real)
    width_own_sim  = float(np.sum(sim_profile  >= peak_frac * sim_avg_peak)  * bin_h_sim)

    # Peak width, cross-referenced against the real peak level (time-above-threshold)
    ref_level = peak_frac * real_avg_peak
    width_ref_real = float(np.sum(real_profile >= ref_level) * bin_h_real)
    width_ref_sim  = float(np.sum(sim_profile  >= ref_level) * bin_h_sim)

    return {
        "peak_time_real": peak_time_real, "peak_time_sim": peak_time_sim,
        "peak_time_offset_min": peak_time_offset_min,
        "load_factor_real": lf_real, "load_factor_sim": lf_sim,
        "peak_width_own_real": width_own_real, "peak_width_own_sim": width_own_sim,
        "peak_width_ref_real": width_ref_real, "peak_width_ref_sim": width_ref_sim,
        "peak_frac": peak_frac,
    }
...
def calculate_validation_metrics(
    ...
    # Relative error metrics: RMSE normalized by the real mean power (nRMSE),
    # with the measured day-to-day coefficient of variation (CV) reported
    # alongside as the natural-variability reference -- an nRMSE at or below
    # CV means the model's error is no larger than the household's own daily
    # fluctuation. LDC-RMSE is normalized the same way for comparability.
    real_mean_power = r["overall_p_mean"]
    nrmse     = (rmse / real_mean_power * 100.0) if real_mean_power > 0 else 0.0
    nldc_rmse = (ldc["ldc_rmse"] / real_mean_power * 100.0) if real_mean_power > 0 else 0.0
    cv_measured = (r["overall_p_std"] / r["overall_p_mean"] * 100.0) if r["overall_p_mean"] > 0 else 0.0

    # Peak-focused metrics (timing, load factor, peak width) on the
    # representative curve -- see calculate_peak_metrics docstring.
    pk = calculate_peak_metrics(real_profile, sim_profile, real_avg_peak, sim_avg_peak, r, s)

    def fmt_err(real_val, sim_val, unit="W"):
        abs_err = abs(real_val - sim_val)
        rel_err = (abs_err / real_val * 100) if real_val > 0 else 0.0
        sep = f" {unit}" if unit else ""
        return f"{abs_err:.2f}{sep} ({rel_err:.1f}%)"
    ...
    rows = [
        ...
        {"Metric Group": "Tier 1: Error", "Metric": "nRMSE (% of real mean power)",
         "Real (Mean ± Std)": "—", "Sim (Mean ± Std)": f"{nrmse:.1f}%", "Mean Abs Error": "—"},
        {"Metric Group": "Tier 1: Error", "Metric": "CV of measured (% of real mean power)",
         "Real (Mean ± Std)": f"{cv_measured:.1f}%", "Sim (Mean ± Std)": "—", "Mean Abs Error": "—"},
        ...
        {"Metric Group": "LDC: Duration-Sorted", "Metric": "nLDC-RMSE (% of real mean power)",
         "Real (Mean ± Std)": "—", "Sim (Mean ± Std)": f"{nldc_rmse:.1f}%", "Mean Abs Error": "—"},
        ...
        {"Metric Group": "Tier 2: Peak", "Metric": "Peak Time of Mean Curve (h)",
         "Real (Mean ± Std)": f"{pk['peak_time_real']:.2f}", "Sim (Mean ± Std)": f"{pk['peak_time_sim']:.2f}",
         "Mean Abs Error": f"{abs(pk['peak_time_offset_min']):.0f} min "
                           f"({'sim later' if pk['peak_time_offset_min'] < 0 else 'sim earlier'})"},
        {"Metric Group": "Tier 2: Peak", "Metric": "Daily Peak-Time Spread (Std, h)",
         "Real (Mean ± Std)": f"{r['peak_time_std']:.2f} (about {r['peak_time_mean']:.1f}h)",
         "Sim (Mean ± Std)": f"{s['peak_time_std']:.2f} (about {s['peak_time_mean']:.1f}h)",
         "Mean Abs Error": fmt_err(r['peak_time_std'], s['peak_time_std'], "h")},
        {"Metric Group": "Tier 2: Peak", "Metric": "Load Factor (mean/peak)",
         "Real (Mean ± Std)": f"{pk['load_factor_real']:.3f}", "Sim (Mean ± Std)": f"{pk['load_factor_sim']:.3f}",
         "Mean Abs Error": fmt_err(pk['load_factor_real'], pk['load_factor_sim'], "")},
        {"Metric Group": "Tier 2: Peak", "Metric": "Peak Width h (>=80% own peak)",
         "Real (Mean ± Std)": f"{pk['peak_width_own_real']:.2f}", "Sim (Mean ± Std)": f"{pk['peak_width_own_sim']:.2f}",
         "Mean Abs Error": fmt_err(pk['peak_width_own_real'], pk['peak_width_own_sim'], "h")},
        {"Metric Group": "Tier 2: Peak", "Metric": "Peak Width h (>=80% real peak)",
         "Real (Mean ± Std)": f"{pk['peak_width_ref_real']:.2f}", "Sim (Mean ± Std)": f"{pk['peak_width_ref_sim']:.2f}",
         "Mean Abs Error": fmt_err(pk['peak_width_ref_real'], pk['peak_width_ref_sim'], "h")},
