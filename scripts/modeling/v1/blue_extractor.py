...
class BlueExtractor(BaseExtractor):
        ...
        for period_name, (start_val, end_val) in periods.items():
            ...
            if len(period_data) > 0:
                ramp_params_by_period['CONS'][period_name] = self._calculate_ramp_params_for_period(
                    period_data, period_name, (start_val, end_val), use_hourly
                )
            else:
    ...
    def _calculate_ramp_params_for_period(self, df_period, period_name, period_bounds=None, use_hourly=True):
        ...
        if len(all_windows) == 0:
            ...
            window_starts = [start for start, _ in all_windows]
            period_length_minutes = self._get_period_length_minutes(period_name, period_bounds, use_hourly)
    ...
    def _get_period_length_minutes(self, period_name, period_bounds=None, use_hourly=True):
