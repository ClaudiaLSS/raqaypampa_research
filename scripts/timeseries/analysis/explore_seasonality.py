"""
Script to explore seasonality patterns in measured load curves using Raqaypampa's agricultural calendar.

Raqaypampa's economy is driven by rain-fed agriculture (secano) across different ecological altitudes.
The seasonal calendar is strictly tied to the agricultural cycle and deeply synchronized with cultural,
musical, and social life. Food security and community traditions depend on this seasonal rhythm.

RAQAYPAMPA AGRICULTURAL SEASONS:
...
Output: Seasonality analysis plots and statistics based on Raqaypampa's agricultural calendar
...
def add_raqaypampa_seasons(df):
    """Add Raqaypampa agricultural season labels based on month.
    
    Seasons:
        Planting (Oct-Jan): Oct, Nov, Dec, Jan - Intensive labor, livestock control, families home
        Growing (Feb-Apr): Feb, Mar, Apr - Constant crop care, harvest preparation, high labor
        Harvesting (May-Jun): May, Jun - Heavy harvesting work, peak labor demands
        Grazing (Jul-Sep): Jul, Aug, Sep - Free-grazing animals, migration possible, low labor
    """
    ...
    df['raq_season'] = df['month'].apply(month_to_season)
    return df


def compute_raqaypampa_season_statistics(df):
    """Analyze load patterns across Raqaypampa's agricultural seasons.
    
    Reflects the actual work cycles and migration patterns in the community.
    """
    print("\n" + "="*80)
    print("RAQAYPAMPA AGRICULTURAL SEASON ANALYSIS")
    print("="*80)
    print("\nSeason Definitions:")
    print("  Planting (Oct-Jan): Intensive labor, livestock control, families home")
    print("  Growing (Feb-Apr): Constant crop care, harvest preparation, high labor")
    print("  Harvesting (May-Jun): Heavy harvesting work, peak labor demands")
    print("  Grazing (Jul-Sep): Free-grazing animals, migration possible, low labor")
...
def plot_monthly_energy_kwh(df, user_id):
    """Calculate and plot total energy consumed (kWh) per month chronologically."""
    print("\nGenerating monthly total energy (kWh) plot...")
    
    # 1. Calculate Energy (kWh) per 5-minute interval
    # Equation: Power (W) * (5 / 60) hours / 1000 = Energy (kWh)
    interval_hours = 5.0 / 60.0
    df['energy_kwh'] = df['p_total'] * interval_hours / 1000.0
    
    # 2. Group chronologically by Year-Month
    df['year_month'] = df['timestamp'].dt.to_period('M')
    monthly_energy = df.groupby('year_month')['energy_kwh'].sum()
    
    # Extract labels for x-axis
    x_labels = monthly_energy.index.astype(str)
    
    # 3. Create the bar chart
    fig, ax = plt.subplots(figsize=(12, 6))
    bars = ax.bar(x_labels, monthly_energy.values, color='teal', alpha=0.8, edgecolor='black')
    
    ax.set_title(f'Total Monthly Energy Consumption (kWh) - User {user_id}', fontsize=14, fontweight='bold')
    ...
    # Label the top of each bar with the exact kWh
    for bar, val in zip(bars, monthly_energy.values):
    ...
    fig_file = FIGURES_DIR / f"seasonality_monthly_energy_kwh_user_{user_id}.png"
    plt.savefig(fig_file, dpi=150, bbox_inches='tight')
    print(f"✓ Saved: {fig_file}")
    plt.close()
...
def plot_raqaypampa_season_comparison(df, user_id):
    """Compare load profiles across Raqaypampa's agricultural seasons."""
    print("\nGenerating Raqaypampa agricultural season comparison plot...")
    ...
    ax.set_title('Migration Indicator: Low Activity Days (< 60 min/day)', 
    ...
    plt.suptitle(f'Raqaypampa Agricultural Seasons Impact on Work Patterns - User {user_id}', 
...
Examples:
  ...
  python explore_seasonality.py --user 74 --no-plots               # Statistics only
    ...
    if not args.no_plots:
        ...
        plot_monthly_energy_kwh(df, args.user)   # <--- The new kWh plot!
