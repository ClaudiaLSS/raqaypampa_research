...
Render every manuscript figure produced by this folder.
    ...
    python run_all.py --only 5 6      # just those figures
...
Figures
    ...
    6  Community daily-trace envelopes (a) and LDC (b), heterogeneous
       vs. homogeneous
...
from figure6_community_ldc_envelope import plot_figure6
...
FILENAMES = {
    ...
    6: "figure6_community_ldc_envelope.png",
}
...
def main():
    ...
    ap.add_argument(
        ...
        default="mean_day",
        help="what Fig. 5 and 6b sort. 'mean_day' (default) sorts the "
    ...
    ap.add_argument(
        ...
        help="Fig. 6a across-day percentile band (use 0 100 for min-max)",
    )
    ap.add_argument(
        "--no-traces", dest="traces", action="store_false",
        help="Fig. 6a: draw only the percentile band, without the "
             "individual daily curves (traces are on by default)",
    )
    ap.add_argument(
        "--community-month", type=int, default=None,
        help="restrict Fig. 6 to one month (default: full year)",
    )
    args = ap.parse_args()

    args.out.mkdir(parents=True, exist_ok=True)
    wants_profiles = any(f in args.only for f in (4, 5))
    wants_community = 6 in args.only
    ...
    if args.demo:
        ...
        month_label, window_label = "mock", "mock"
    else:
        import data_io
        ...
        # decides how strong Fig. 6's contrast looks (see the PREMISE notes
    ...
    if 6 in args.only:
        plot_figure6(
            community_df, args.out / FILENAMES[6],
